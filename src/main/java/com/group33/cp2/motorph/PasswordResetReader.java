...
public class PasswordResetReader {
    ...
    /**
     * Appends a new password reset request row to the CSV.
     *
    ...
    /**
     * Returns all password reset requests (header excluded).
     *
     * @return list of all requests; empty list if none
     */
    ...
    /**
     * Marks the most recent Pending request for the given employee as Approved.
     *
     * @param employeeNumber the employee whose request to approve
     * @param adminName      the IT admin's full name
     * @param adminEmpNum    the IT admin's employee number
     * @return {@code true} if a pending request was found and updated
     * @throws IOException if the file cannot be written
     */
    public static boolean approveRequest(String employeeNumber, String adminName, String adminEmpNum)
        ...
        for (int i = 1; i < rows.size(); i++) {
            ...
            if (row.length > 0 && row[0].trim().equals(employeeNumber.trim())
                ...
                // Extend row to 7 elements if needed
