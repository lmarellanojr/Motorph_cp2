...
public class SalaryDetailsReader {
    ...
    // null means "not yet loaded"; reload is triggered on next read call
    ...
    /** Returns the {@link Salary} for the given employee ID, or {@code null} if not found. */
    public Salary getSalary(String empId) {
        if (cache == null) loadCache();
        return cache.get(empId.trim());
    }

    /** Returns an unmodifiable view of all salary entries (empId → Salary). */
    public Map<String, Salary> getAllSalaries() {
        if (cache == null) loadCache();
        return Collections.unmodifiableMap(cache);
    }

    /** Appends a new salary row for the given employee ID. */
    ...
    /** Updates the salary row for the given employee ID. */
    ...
    /** Removes the salary row for the given employee ID. */
    public void deleteSalary(String empId) throws IOException {
        List<String[]> all = readAll();
        // header row[0] is "employeeNum" — won't match a real ID
